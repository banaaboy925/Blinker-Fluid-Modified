...
bool SupportsAppleGPUFamily(id<MTLDevice> device, uint8_t appleFamily)
{
    ...
    // supportsFamily: is unavailable on iOS 12 even though current SDKs expose
    // it at compile time. The first feature set for each family is the legacy
    // API's equivalent capability test.

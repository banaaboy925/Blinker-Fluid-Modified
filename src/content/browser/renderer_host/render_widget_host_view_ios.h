...
class CONTENT_EXPORT RenderWidgetHostViewIOS
 ...
 public:
  ...
  // Returns the focused caret's bottom edge as a viewport-height ratio, or -1
  // when renderer metrics are unavailable.
  float GetFocusedInputBottomRatio();

  // Resolves the focused editable in an isolated world and returns the bottom
  // edge of its input container as a viewport-height ratio. The callback
  // receives -1 when no usable element is focused.
  void RequestFocusedInputBottomRatioFromDOM(

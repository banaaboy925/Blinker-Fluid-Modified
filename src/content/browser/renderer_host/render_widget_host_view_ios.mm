...
bool RenderWidgetHostViewIOS::RequiresDoubleTapGestureEvents() const {
  // Deliver taps immediately. Holding the first tap for double-tap-to-zoom
  // adds latency and prevents sites that use two DOM clicks (for example file
  // managers) from recognizing their own double-click action.
  return false;
}
...
// Both functions return before BrowserEngineKit access on pre-17.4 systems.

...
@implementation WebMenuRunner {
  ...
  // Native action sheet used by iOS 12, before UIContextMenuInteraction.
  UIAlertController* __strong _legacyMenuController;
  NSMutableArray<NSDictionary*>* __strong _legacyMenuItems;
...
- (void)showMenuInView:(UIView*)view withBounds:(CGRect)bounds {
  ...
  if (!@available(iOS 13.0, *)) {
    _legacyMenuController =
        [UIAlertController alertControllerWithTitle:nil
                                            message:nil
                                     preferredStyle:UIAlertControllerStyleActionSheet];
    for (NSDictionary* item in _legacyMenuItems) {
      const size_t index = [item[@"index"] unsignedIntegerValue];
      UIAlertAction* action =
          [UIAlertAction actionWithTitle:item[@"title"]
                                   style:UIAlertActionStyleDefault
                                 handler:^(__unused UIAlertAction* selected) {
                                   self->_menuItemWasChosen = YES;
                                   self->_selectedIndex = index;
                                   if (self->_delegate) {
                                     self->_delegate->OnMenuItemSelected(index);
                                   }
                                 }];
      [_legacyMenuController addAction:action];
    }
    [_legacyMenuController
        addAction:[UIAlertAction actionWithTitle:@"Cancel"
                                        ...
                                        handler:^(__unused UIAlertAction* action) {
                                          if (self->_delegate) {
                                            self->_delegate->OnMenuCanceled();
                                          }
                                        }]];
    UIPopoverPresentationController* popover =
        _legacyMenuController.popoverPresentationController;
    ...
    [controller presentViewController:_legacyMenuController
                             animated:YES
                           completion:nil];
    return;
  }
...
- (void)dismissMenu {
  if (!@available(iOS 13.0, *)) {
    [_legacyMenuController dismissViewControllerAnimated:YES completion:nil];
    _legacyMenuController = nil;
    return;
  }
...
- (void)createMenu:(const std::vector<blink::mojom::MenuItemPtr>&)items {
  if (!@available(iOS 13.0, *)) {
    _legacyMenuItems = [NSMutableArray array];
    for (size_t i = 0; i < items.size(); ++i) {
      ...
      [_legacyMenuItems
          addObject:@{
            @"title" : base::SysUTF8ToNSString(items[i]->label.value_or("")),
            @"index" : @(i),
          }];
    }
    return;

...
void InProcessGpuThread::Init() {
...
#if BUILDFLAG(IS_IOS) && !BUILDFLAG(IS_IOS_TVOS)
  BlinkBootLog("GPU0: InProcessGpuThread::Init, creating transport");
  be_layer_transport_ =
      std::make_unique<InProcessGpuThread::BELayerHierarchyTransportImpl>();
  BlinkBootLog("GPU0b: transport instance set");
#endif
  ...
  gpu_init->InitializeInProcess(base::CommandLine::ForCurrentProcess(),
                                gpu_preferences_);
  BlinkBootLog("GPU0d: InitializeInProcess returned (GL/ANGLE init done)");

...
size_t MaxStreamBytesInMemory(BlobStorageContext* context) {
#if BUILDFLAG(IS_IOS)
  return 4 * 1024 * 1024;
#else
  return context->memory_controller().limits().min_page_file_size;
#endif
}
...
BlobBuilderFromStream::BlobBuilderFromStream(
    ...
    : kMemoryBlockSize(std::min(
      ...
      // iOS has a single-process renderer and a much tighter practical memory
      // ceiling. Streamed uploads accumulated hundreds of megabytes of blob
      // chunks and eventually failed inside AllocateBytes(). Keep only a small
      // prefix in RAM there and page the remainder to the sandboxed temp store.
      kMaxBytesInMemory(MaxStreamBytesInMemory(context.get())),
...
#if BUILDFLAG(IS_IOS)
  // If paging could not be initialized, fail a pathological upload cleanly
  // instead of letting an unbounded stream terminate the whole browser.
  constexpr uint64_t kIOSInMemoryStreamHardLimit = 64 * 1024 * 1024;
  if (!context_->memory_controller().file_paging_enabled() &&
      (current_total_size_ >= kIOSInMemoryStreamHardLimit ||
       length_hint > kIOSInMemoryStreamHardLimit)) {

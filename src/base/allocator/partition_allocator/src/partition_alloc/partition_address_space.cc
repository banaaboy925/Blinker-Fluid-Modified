...
bool PartitionAddressSpace::IsIOSTestProcess() {
  ...
  // iOS-15 port: this jailbroken device does NOT effectively honor the
  // extended-virtual-addressing entitlement, so content_shell gets only ~4 GB
  // of reservable address space — exactly the constraint described above for
  // test processes. Force the small pool size for ALL processes so
  // PartitionAlloc's multi-GB pool reservation fits (otherwise PartitionRoot
  // init traps in HandlePoolAllocFailure at the first malloc).
  return true;

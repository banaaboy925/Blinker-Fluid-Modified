...
bool PlatformMimeUtil::GetPlatformMimeTypeFromExtension(
  ...
  if (@available(iOS 14.0, *)) {
    ...
    // non-identifiable mime type. Treat them as a nonstandard format.
  ...
  base::apple::ScopedCFTypeRef<CFStringRef> identifier(
      UTTypeCreatePreferredIdentifierForTag(
          kUTTagClassFilenameExtension,
          base::SysUTF8ToCFStringRef(ext_nodot).get(), nullptr));
  if (!identifier || UTTypeIsDynamic(identifier.get())) {
    return false;
  }
  base::apple::ScopedCFTypeRef<CFStringRef> mime_type(
      UTTypeCopyPreferredTagWithClass(identifier.get(), kUTTagClassMIMEType));
  if (!mime_type) {
    return false;
  }
  *result = base::SysCFStringRefToUTF8(mime_type.get());
  return true;
}
...
bool PlatformMimeUtil::GetPlatformPreferredExtensionForMimeType(
  ...
  base::apple::ScopedCFTypeRef<CFStringRef> identifier(
      UTTypeCreatePreferredIdentifierForTag(
          kUTTagClassMIMEType, base::SysUTF8ToCFStringRef(mime_type).get(),
          nullptr));
  if (!identifier || UTTypeIsDynamic(identifier.get())) {
    return false;
  }
  base::apple::ScopedCFTypeRef<CFStringRef> extension(
      UTTypeCopyPreferredTagWithClass(identifier.get(),
                                      kUTTagClassFilenameExtension));
  if (!extension) {
    return false;
  }
  *ext = base::SysCFStringRefToUTF8(extension.get());
  return true;
}
